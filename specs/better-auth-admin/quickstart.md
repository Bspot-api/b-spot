...
# Email (already present — add SMTP_FROM if missing)
SMTP_FROM="B-Spot <noreply@bspot.fr>"
```
...
curl -X POST http://localhost:3000/api/auth/sign-in/magic-link \
  ...
  -d '{"email": "bspot.api@gmail.com", "callbackURL": "http://localhost:3000/api/admin/me"}'
```

Check your SMTP inbox (or Mailhog if configured locally) for the link.
...
```bash
curl -X DELETE http://localhost:3000/api/admin/admins/<userId> \
  -H "Cookie: session_token=<admin-token>"
...
Admin routes are documented at `http://localhost:3000/api/docs` under the `admin` tag.
